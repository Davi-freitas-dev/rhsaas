...
class DemoPublicFlowTests(MultiTenantTestCase):
    ...
    def test_fluxo_publico_aloca_e_autentica_sem_senha(self):
        ...
        self.assertTrue(exchange_payload["user"]["canViewServers"])
        self.assertTrue(exchange_payload["user"]["canViewServerParticipations"])
        self.assertTrue(exchange_payload["user"]["canViewServerCosts"])
        self.assertTrue(exchange_payload["user"]["canAddServer"])
        self.assertTrue(exchange_payload["user"]["canChangeServer"])
        self.assertTrue(exchange_payload["user"]["canManageServerParticipation"])
        self.assertTrue(exchange_payload["user"]["canRecalculateServerCosts"])
        self.assertTrue(exchange_payload["user"]["canViewServerSalary"])
        self.assertTrue(exchange_payload["user"]["canChangeServerSalary"])
        self.assertTrue(exchange_payload["user"]["canViewServerSensitiveData"])
        self.assertTrue(exchange_payload["user"]["canChangeServerDistributedValue"])
        self.assertTrue(exchange_payload["user"]["canViewServerAppropriation"])
        ...
        with schema_context("demo2"):
            ...
            self.assertTrue(user.has_perm("caixa.view_servidor"))
            self.assertTrue(user.has_perm("caixa.view_participacaoservidorevento"))
            self.assertTrue(user.has_perm("caixa.view_planocustorecorrente"))
            self.assertTrue(user.has_perm("caixa.materialize_planocustorecorrente"))
